...
function Login() {
  ...
  return (
    <>
      ...
      <div className='w-[20rem] h-[27rem] absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-white bg-opacity-65 flex items-center justify-center flex-col backdrop-blur-sm'>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className='flex items-center justify-center flex-col w-[65%]'>
          ...
          />
          {errors.secretKey && <span className='text-[0.80rem] text-red-500'>{errors.secretKey.message}</span>}
          <div className='text-[0.80rem] text-white w-full'>
            Don't have an account? <Link to='/admin/signup' className='text-custom-primary font-semibold hover:text-red-300'>Register</Link>
          </div>
