...
import signupImage from '../../assets/adminsignupimage.jpg'
...
function Signup() {
  ...
  const handleSubmit = async(e) => {
    ...
    if(emptyFields){
      ...
      try{
        const response = await axiosInstance.post('/admin/signup', user);
  ...
  return (
    <>
      ...
      <div className='w-[22rem] h-[31rem] absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 rounded-lg backdrop-blur-sm border border-white bg-opacity-65 flex items-center justify-center flex-col'>
        ...
        <form className='flex items-center justify-center flex-col w-[65%]' onSubmit={handleSubmit}>
          ...
          <div className='text-[0.80rem] text-white w-full'>Already have an account? <Link to='/admin/login' className='text-custom-primary font-semibold hover:text-red-300'>Login</Link></div>
